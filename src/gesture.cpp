...
void PlaySound(string file)
{
	...
	if (strlen(file.c_str()) > 200) return;	// バッファオーバーフロー防止
...
vector<int> k_curvature(vector<Point> contour, vector<Vec4i> defects, int k, float threshold)
{
  ...
  vector<int> one_fin_defects;
	...
	for (int i = 0; i < defects.size(); i++) {
		for(int j=-5; j<=5; j++){
      //if(defects[i][2] - k + j >=0 && defects[i][2] + k + j < contour.size() ){
...
int main(int argc, char *argv[])
{
    ...
    while (loop_flag)
  {
    ...
    //cv::bilateralFilter(frame, frame, -1, 50, 12);
    if(faces.size()==0){
    ...
    absdiff(msk_img, msk_back, diff); //背景画像との差分を取得
    cv::medianBlur(diff, diff, 21);

   // dst_img = cv::Scalar(0);

    // cv::Moments mu = cv::moments( diff, false );
    // cv::Point2f mc = cv::Point2f( mu.m10/mu.m00 , mu.m01/mu.m00 );
    // circle( diff, mc, 4, cv::Scalar(100), 2, 4);//重心(白色)
...
if(maxId>=0){
  ...
  // circle( diff, mc1, 4, cv::Scalar(0, 0, 100), 2, 4);//重心（輪郭）
  //printf("%d\n",maxId);
  // if(maxId>=0){
  //   int length = contours[maxId].size();
  //   for(int i=0;i<length;i++){
  //     cv::Point point = contours[maxId][i];
  //     circle(diff, point, 4, cv::Scalar(100), 2, 4);//輪郭
  //   }
  // }
  ...
  // if(enclose.second>0){
  //   circle(diff, enclose.first, enclose.second, cv::Scalar(100), 2, 4);//内接円
  // }else continue;
  ...
  if(fin==0){
    ...
    // for(int i=0;i<num_defects ;i++){
    // circle(diff,contours[maxId][defects[i][1]], 4, cv::Scalar(100,0,0),2,4);
    // }
  ...
  pre_most_common = most_common;
    ...
    if (key == 27)
    {
      ...
      first = 0;
        // cv::cvtColor(real_pure_frame, hsv_back, cv::COLOR_BGR2HSV);
        // cv::inRange(hsv_back, cv::Scalar(0, 30, 60), cv::Scalar(20, 150, 255), msk_back);
    }
  }
  return 0;
